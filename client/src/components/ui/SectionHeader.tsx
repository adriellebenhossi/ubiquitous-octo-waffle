  ...
  const createMinimalistBadge = () => {
    ...
    if (sectionKey && sectionColors && sectionColors[sectionKey]) {
      ...
      if (sectionConfig.gradientColors && sectionConfig.gradientColors.length > 0) {
        ...
        // Se a primeira cor é muito clara (como #ffffff), verificar a segunda cor
        if (badgeColor === '#ffffff' || badgeColor === '#f8fafc' || badgeColor.toLowerCase().includes('fff')) {
          if (sectionConfig.gradientColors.length > 1) {
            const secondColor = sectionConfig.gradientColors[1];
            // Verificar se a segunda cor também é muito clara
            if (secondColor === '#ffffff' || secondColor === '#f8fafc' || secondColor.toLowerCase().includes('fff')) {
              // Se ambas as cores são claras, usar quoteIconColor
              if (sectionConfig.quoteIconColor) {
    ...
    // Estilos minimalistas - apenas cor do texto e ícone
    const badgeStyles = {
      color: badgeColor,
    };
    ...
    return (
      <div className="flex items-center justify-center mb-6">
        <div 
          className="inline-flex items-center gap-2 font-medium text-xs uppercase tracking-widest transition-all duration-200"
          style={badgeStyles}
        >
          {BadgeIcon && (
            <BadgeIcon 
