          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            ...
            <div className="flex items-center gap-4 lg:gap-6">
              ...
              <button
                ...
                className="flex items-center gap-1 px-2 py-1 lg:px-2 lg:py-1 bg-gray-900 hover:bg-gray-800 text-white rounded text-xs font-medium transition-all duration-300 shadow-sm hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed group"
                disabled={saveIconMutation.isPending}
                data-testid="button-toggle-icon-selector"
              >
                <span className="hidden sm:inline">{showSelector ? 'Fechar' : 'Alterar ícone'}</span>
                <span className="sm:hidden">{showSelector ? 'Fechar' : 'Alterar'}</span>
                <ChevronDown className={`w-2.5 h-2.5 transition-transform duration-300 group-hover:scale-110 ${showSelector ? 'rotate-180' : ''}`} />
              </button>
            </div>
          </div>
